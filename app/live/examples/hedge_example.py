...
from ..logic.hedging import HedgeEngine
from ..logic.models import HedgeConfig, HedgeMode, HedgeExecutionParams, HedgeOffsetsPctX10000
from ..logic.models import HedgeVolumeRequest, HedgeLeg
...
def parse_args():
    ...
    # Quote notional (passed into callback)
    p.add_argument('--quote', type=float, required=True)

    # Percents (ordinary floats)
...
def main():
    ...
    # Position forces one-way mode: exchange hedge_mode must be False
    ...
    # Callback: quote notional -> base_units using current price from request
    def on_volume(req: HedgeVolumeRequest) -> int:
        ...
        if base_units <= 0:
            raise RuntimeError(f'on_volume: bad base_units: {base_units} from quote={quote} price_float={price_float}')
    ...
    try:
        ...
        if main_exc is not None and len(cleanup_errors) > 0:
            _t, exc, _tb = main_exc
            raise RuntimeError(f'Hedge example failed and cleanup failed too: cleanup_errors={cleanup_errors}') from exc
